...
class MainWindow(TemplateBaseClass):
            ...
            if ext in ["phd"]:
                ...
                for i in range(self.parser.no_of_curves()):
                    ...
                    x, y = get_x_y(
                        i,
                        self.parser,
                        smooth_trace=self.ui.smooth_checkBox.isChecked(),
                        boxwidth=self.ui.smooth_spinBox.value(),
                    )
                    self.x_i.append(x)
                    self.y_i.append(y)
    ...
    def add_trace_to_temp_plot(self):
        try:
            ...
            self.temp_plot.plot(
                self.x_i[self.ui.comboBox.currentIndex()],
                self.y_i[self.ui.comboBox.currentIndex()],
                pen="r",
            )
    ...
    def pub_ready_plot_export(self):
        try:
            filename = QtWidgets.QFileDialog.getSaveFileName(
                self, caption="Filename with EXTENSION"
            )
            ...
            for i in range(len(self.x_mem)):
                if self.ui.Normalize_checkBox.isChecked():
                    plt.plot(
                        self.x_mem[i],
                        self.y_mem[i] / max(self.y_mem[i]),
                        label=str(self.legend[i]),
                    )
                else:
