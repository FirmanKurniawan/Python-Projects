...
def mahasiswa(keyword):
    ...
    x = requests.get(
        "https://api-frontend.kemdikbud.go.id/hit_mhs/" + keyword, headers=headers
    ).text
    output = json.loads(x)
    ...
    for s in output["mahasiswa"]:
        if "Cari kata kunci" in s["text"]:
            ...
            get_detail = requests.get(
                "https://api-frontend.kemdikbud.go.id/detail_mhs/" + code,
                headers=headers,
            ).text
            ...
            if keyword.isdecimal():
                print(
                    ...
                    + detail_data["namapt"]
                    + "\nJenjang\t\t: "
                    + detail_data["namajenjang"]
                    + "\nProdi\t\t: "
                    + detail_data["namaprodi"]
                    + "\nSemester Awal\t: "
                    + detail_data["mulai_smt"][:-1]
                    + "\nURL\t\t: https://pddikti.kemdikbud.go.id/data_mahasiswa/"
                    + code
                    + "\n"
                )
            else:
                if keyword.lower() in detail_data["nm_pd"].lower():
                    print(
                        ...
                        + detail_data["mulai_smt"][:-1]
                        + "\nLulus\t\t: "
                        + str(detail_data["ket_keluar"])
                        + "\nTanggal Keluar\t: "
                        + str(detail_data["tgl_keluar"])
                        + "\nNo. Ijazah\t: "
                        + str(detail_data["no_seri_ijazah"])
                        + "\n"
                    )
