...
def add_notes():
    ...
    if (len(today) <= 0) & (len(notes_title) <= 0) & (len(notes) <= 1):
        ...
        cur.execute(
            "INSERT INTO notes_table VALUES ('%s','%s','%s')"
            % (today, notes_title, notes)
        )
...
def view_notes():
    ...
    if (len(date) <= 0) & (len(notes_title) <= 0):
        ...
        sql_statement = (
            "SELECT * FROM notes_table where notes_title ='%s'" % notes_title
        )
        ...
        sql_statement = (
            "SELECT * FROM notes_table where date ='%s' and notes_title ='%s'"
            % (date, notes_title)
        )
...
def delete_notes():
    ...
    if choice == "yes":
        ...
        if (len(date) <= 0) & (len(notes_title) <= 0):
            ...
            sql_statement = (
                "DELETE FROM notes_table where date ='%s' and notes_title ='%s'"
                % (date, notes_title)
            )
...
def update_notes():
    ...
    if (len(today) <= 0) & (len(notes_title) <= 0) & (len(notes) <= 1):
        ...
        sql_statement = (
            "UPDATE notes_table SET notes = '%s' where date ='%s' and notes_title ='%s'"
...
button1 = Button(
    window, text="Add Notes", bg="Turquoise", fg="Red", command=add_notes
).place(x=10, y=190)
button2 = Button(
    window, text="View Notes", bg="Turquoise", fg="Red", command=view_notes
).place(x=110, y=190)
button3 = Button(
    window, text="Delete Notes", bg="Turquoise", fg="Red", command=delete_notes
).place(x=210, y=190)
button4 = Button(
    window, text="Update Notes", bg="Turquoise", fg="Red", command=update_notes
).place(x=320, y=190)
