...
def message_received(client, server, message):
    ...
    if message == "check_wa":
        ...
        if method == "APPIUM":
            ...
            helpersny.createJob("INSERT INTO jobs_data ('acc_id', 'job_type', 'status_job', 'method') VALUES ("+account_id+", "+job_type+", 0, 'APPIUM')")
        else:
            print("FULL ADB")
            target_no = split[3]
            message = split[4]
            ...
            try:
                ...
                filename = split[5]
                print("SINI")
                check_job = helpersny.createJob("INSERT INTO jobs_data ('acc_id', 'job_type', 'target', 'filename', 'status_job', 'message', 'method') VALUES ("+account_id+", "+job_type+", '"+target_no+"', '"+filename+"', 0, '"+message+"', 'ADB FULL')")
            except:
                print("EXPECT")
                check_job = helpersny.createJob("INSERT INTO jobs_data ('acc_id', 'job_type', 'target', 'status_job', 'message', 'method') VALUES ("+account_id+", "+job_type+", '"+target_no+"', 0, '"+message+"', 'ADB FULL')")
    print("Client(%d) said: %s" % (client['id'], message))
