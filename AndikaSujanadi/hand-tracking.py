...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:  # Getting Landmark(location) of Hands if Exists
        ...
        for (
            hand_in_frame
        ) in results.multi_hand_landmarks:  # looping through hands exists in the Frame
            mpDraw.draw_landmarks(
                img, hand_in_frame, mpHands.HAND_CONNECTIONS
        ...
        if lmList[thumbCoordinate[0]][0] > lmList[thumbCoordinate[1]][0]:
            upcount += 1
        cv2.putText(
            img, str(upcount), (150, 150), cv2.FONT_HERSHEY_PLAIN, 12, (0, 0, 255), 12
        )
