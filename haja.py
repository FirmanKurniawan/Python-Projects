...
def _rclear(x):
    x["_RETURN_STACK"] = {"TOP": [None], "SUB": [None], "REST": []}
...
def _cold(x):
    ...
    PY["_clear"](x)
    PY["_rclear"](x)
    x.update({"py": x["_Python"], "r": x["_RETURN_STACK"], "s": x["_DATA_STACK"]})
...
def drop(x):
    ...
    y = S["TOP"][0]
    S["TOP"][0] = S["SUB"][0]
    S["SUB"][0] = x["_Python"]["_DEFAULT"] if not S["REST"] else S["REST"].pop()
    return y
...
def eval_(x):
    PY = x["_Python"]
    PY["push"](x, eval(PY["drop"](x), eval(x["_EXTERNAL"])["globals"], PY))
...
def exec_(x):
    exec(x["_Python"]["drop"](x), eval(x["_EXTERNAL"])["globals"], x["_Python"])
...
def _clear(x):
    x["_DATA_STACK"] = dict(
        map(lambda name: (name, x["_Python"]["defaultlist"](x)), "TOP SUB REST".split())
    )
...
def forth(x):
    PY = x["_Python"]
    PY["push"](x, PY["_DEFAULT"] if not x["_BACK_STACK"] else x["_BACK_STACK"].pop())
...
def preprocess(string):
    ...
    def splitIgnoringGraphicSpace(string):
        ...
        return tuple(map(lambda s: s.replace(_, " "), string.replace("\\ ", _).split()))
...
def semicolon(x):
    WORD = x["_WORD_UNDER_CONSTRUCTION"][0]
    x["_DATA"].update({WORD: tuple(reversed(x["_DATA"][WORD]))})
