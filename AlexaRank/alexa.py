...
def stats():
    ...
    r = requests.get(
        "https://data.alexa.com/data?cli=10&dat=snbamz&url={}".format(url), verify=False
    )
...
def mass():
    ...
    for i in yaa:
        boo = i.strip()
        r = requests.get(
            "https://data.alexa.com/data?cli=10&dat=snbamz&url={}".format(boo),
            verify=False,
        )
        alexa = re.findall('TEXT="(.*)" SOURCE="panel"', r.text)
