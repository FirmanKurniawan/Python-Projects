...
def TTS():
    ...
    Label(
        tts_wn, text="Text-to-Speech Converter", font=("Comic Sans MS", 15), bg="Brown"
    ).place(x=50)
...
def STT():
    ...
    record_btn = Button(
        stt_wn, text="Record", bg="Sienna", command=lambda: text.insert(END, record())
    )
...
Label(
    root,
    text="ProjectGurukul Text-To-Speech and Speech-To-Text Converter",
...
tts_btn = Button(
    root, text="TTS Conversion", font=("Helvetica", 16), bg="MediumPurple", command=TTS
)
tts_btn.place(x=60, y=150)

stt_btn = Button(
    root, text="STT Conversion", font=("Helvetica", 16), bg="MediumPurple", command=STT
)
stt_btn.place(x=60, y=200)

instruction_btn = Button(
    root,
    text="Instructions before starting",
    font=("Helvetica", 16),
    bg="MediumPurple",
    command=instruction,
)
