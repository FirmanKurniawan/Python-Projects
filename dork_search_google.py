...
try:
    ...
    def Exploit(dork, total_page):
        ...
        for b in googlesearch.search(dork, num=Total_page):
            web = (
                b + "'"
            )  # add ' to end the result url. to check if website is vuln by SQL Injection
            # using requests
            ...
            for Type, ErrorMessage in errors.items():
                if re.search(ErrorMessage, webs):
                    ...
                    print(
                        " \033[41m\033[30mVULN\033[40m\033[37m {0}\n Vulnerability Type: \033[31m{1}".format(
                            b, Type
                        )
                    )
    ...
    while True:
        ...
        if not dork:
            Exploit(dork="inurl:cart.php?id=", total_page=total_page)
        else:
